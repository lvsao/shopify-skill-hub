...
import { shopifyGraphql, loadShopifyConfig } from "./lib/shopify-dev-dashboard-auth.mjs";
...
function usage() {
  console.log(`Usage:
  node shopify-blog-seo-admin.mjs init-env --method dev_dashboard_client_credentials --env skill-hub.env
...
async function publicUrlSignals(rawUrl) {
  try {
    ...
    return {
      ...
      passwordPage: /\/password(?:["'/?#]|$)/i.test(response.url),
    };
  } catch (error) {
...
function auditHtml(article) {
  ...
  const links = [...body.matchAll(/<a\b([^>]*)>/gi)].map((match) => ({ href: match[1].match(/\bhref=["']([^"']*)["']/i)?.[1] ?? null, text: stripHtml(match[1]) }));
...
function renderReport(plan) {
  ...
  const body = safeArticleHtml(candidate.body || article.body || "");
  ...
  const research = (plan.research || []).map((item) => `<li><a href="${escapeHtml(item.url || "#")}">${escapeHtml(item.title || item.publisher || item.url || "Source")}</a> — ${escapeHtml(item.supports || item.note || "")}</li>`).join("") || "<li>No research sources attached.</li>";
  ...
  const imageMarkup = article.image?.url ? `<img src="${escapeHtml(article.image.url)}" alt="${escapeHtml(article.image.altText || "")}">` : "";
...
async function initEnv(args) {
  ...
  console.log(JSON.stringify({ ok: true, env: output, method, requiredScopes: "read_content|read_online_store_pages,write_content|write_online_store_pages" }, null, 2));
}
...
async function main() {
  ...
  if (command === "report") { const plan = await readJson(args.input || "-"); const output = args.output || `shopify-blog-seo-report-${Date.now()}.html`; await fs.writeFile(output, renderReport(plan), "utf8"); console.log(JSON.stringify({ ok: true, output: path.resolve(output) }, null, 2)); return; }
  const env = await loadShopifyConfig(args.env || DEFAULT_ENV);
  if (command === "connection-check") { console.log(JSON.stringify(await shopifyGraphql(env, SHOP_QUERY), null, 2)); return; }
  ...
  if (command === "audit") {
    ...
    const value = { article: { ...result.article, body: result.article.body }, audit, preview: { mode: "theme-like fallback", storefrontUrl: result.article.storefrontUrl }, status: "audit-complete" };
  ...
  if (command === "apply") {
    ...
    for (const key of ["body", "summary"]) if (updates[key] !== undefined) article[key] = String(updates[key]);
  ...
  if (command === "verify") {
    ...
    console.log(JSON.stringify({ article: { ...data.article, body: undefined }, audit: auditHtml(data.article) }, null, 2));
    return;
  }
  usage();
  process.exitCode = 1;
