...
<!-- onboarding-contract: 1.0.0; source-sha256: e80a0b5ef7ed242d3b11086506b0d715751b7440bf42b684b24b51abb18c7a6f -->
# Connect Your Store

Use this guide only when the merchant asks to scan or apply Shopify variant barcodes; local generation requires no connection.

## Choose the smallest access path

Local GTIN-14 generation works without Shopify credentials. A store connection is required only to scan variants or perform an explicitly approved update.
...
Connection never replaces the scan → CSV review → explicit approval → `apply --execute` → verification boundary. Existing barcodes are not changed by a scan.
