...
<!-- onboarding-contract: 1.0.0; source-sha256: 2521aa4eef5c1ded4d93822ab6067bceb4963d5a3ee497a88022b41272cf8860 -->
# Connect Your Store

Use this guide before the connected checkout, delivery, and payment checks.

## Choose the smallest access path

This skill needs Shopify Admin read access for payment and delivery evidence. The public storefront walkthrough supplements that evidence; it never replaces authorization.
...
```text
shopify store auth --store <shop>.myshopify.com --scopes read_orders,read_shipping,read_shopify_payments_accounts --json
```
...
The Admin-data portion is read-only and never creates an order, product, customer, or payment. The browser walkthrough may create a temporary unsubmitted checkout session, and always stops before payment confirmation.
