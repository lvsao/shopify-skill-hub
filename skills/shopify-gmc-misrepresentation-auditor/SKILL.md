...
### A7 — AI Bulk / Dropshipping Detection

Automated checks that detect signals of AI-bulk-generated or dropshipping storefronts. Three dimensions assessed together.

**Dimension 1 — Domain age × catalog density (automated):**
| Product count | Domain age (if known) | Signal |
|-------------|----------------------|--------|
| < 50 | Any | No trace |
| 50–100 | > 12 months | No trace |
| 50–100 | < 6 months | Suspected |
| 100+ | > 12 months | Suspected |
| 100+ | < 6 months | High likelihood |

If the store uses a `.myshopify.com` subdomain (no custom domain), treat domain age as unknown — use product count as the primary signal.

**Dimension 2 — Product sourced from supplier catalogs (agent-assisted):**
The automated script flags supplier-language patterns (China origin, spec measurements in cm/kg, wholesale terms, "Package includes" boilerplate). For confirmation, the agent MUST:
- Pick 2–3 sampled product titles and search for them on AliExpress, CJdropshipping, Temu, or DHGate
- Compare product description text for near-verbatim matches
- Report findings (confirmed / suspected / not found)

**Dimension 3 — Brand search presence (agent-assisted):**
The agent MUST search for the store name / brand and assess:
- Any meaningful web presence beyond the store itself?
- Any reviews, social media, press coverage?
- Is the brand name registered and findable?

| Check | Method | Severity |
|-------|--------|----------|
| Custom domain | Check if URL is `.myshopify.com` vs custom domain | Medium |
| Catalog density | Compare product count vs brand age signals | Medium |
| Domain WHOIS | Check domain creation date via whois API for age signals | Medium |
| Supplier text patterns | Regex for China origin, spec measurements, wholesale boilerplate | Medium |
| Product description thinness | Count words per product page (< 50 words flag) | Medium |
| [Agent] Product sourced from AliExpress/CJ/Temu | Search sampled product titles | High |
| [Agent] Brand search presence | Search store/brand name online | High |
...
| PQ-15 | Supplier/dropshipping patterns in product description | Medium |
| PQ-16 | Thin product description (< 50 words) — AI-bulk signal | Medium |

## Risk Scoring

```
RiskScore = min(100, StoreRaw + AvgProductRaw × ScopeMultiplier)
```

Where:
- `StoreRaw` = Σ(SeverityWeight × Confidence × EvidenceFactor) for store-level findings only
- `ProductRaw` = Σ(SeverityWeight × Confidence × EvidenceFactor) for all product findings combined
- `AvgProductRaw` = `ProductRaw` / number of products audited (normalized to avoid linear catalog scaling)
- `ScopeMultiplier` applies when patterns affect many products: 1.0 (single), 1.5 (collection ≥30%), 2.5 (store-wide ≥60%)
...
1. Header — store URL, scan date, risk score, finding counts
2. Key metrics — critical/high/medium counts, products scanned
3. Critical & High findings — must-fix items with evidence
4. Advisory findings — Medium & Low issues (deduplicated with counts)
5. Policy pages discovered — URLs and canonical status
6. **AI Bulk / Dropshipping Assessment** — automated signals table + agent-assisted guidance
7. Product-level detail — per-product results with findings
8. Systemic patterns — cross-product issue signatures
9. **Manual Checklist** — MC-01 through MC-07 as interactive checkboxes
10. Staged remediation plan — Phase 1 (today) through Phase 4 (ongoing)
11. Technical appendix — theme detected, JSON-LD details, selectors used
