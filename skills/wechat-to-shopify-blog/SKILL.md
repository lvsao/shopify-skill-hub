...
- **Encoding/decoding safety (CRITICAL):** All file operations must explicitly use `'utf8'` encoding in Node.js. When reading the WeChat article (which may contain Chinese characters), always specify encoding explicitly. When writing article body HTML, draft JSON, or image manifests, use `writeFileSync(path, content, 'utf8')`. On macOS and Linux UTF-8 is the default terminal encoding; on Windows, PowerShell may default to system code page — always explicitly set encoding. After writing any file, re-read it and verify Chinese characters are not garbled (look for `\uFFFD`, `�`, or broken multi-byte sequences). When calling Shopify CLI on Windows, always use `--query-file` and `--output-file` to avoid PowerShell quote/encoding corruption.
...
```text
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-context.mjs --env skill-hub.env
```
...
```text
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs init-env --method admin_custom_app --env skill-hub.env
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs init-env --method dev_dashboard_app --env skill-hub.env
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-context.mjs --env skill-hub.env --product-page-size 50
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\fetch-wechat-article.mjs --url <mp.weixin.qq.com URL>
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\fetch-wechat-article.mjs --url <mp.weixin.qq.com URL> --download-images --output-dir <temporary-existing-or-disposable-folder>
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs context --env skill-hub.env --product-page-size 50
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs upload-images --env skill-hub.env --input image-manifest.json
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs upload-images --env skill-hub.env --input image-manifest.json --execute
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs create-draft --env skill-hub.env --input draft-article.json --require-images
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs create-draft --env skill-hub.env --input draft-article.json --execute --require-images
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs update-draft --env skill-hub.env --article-id gid://shopify/Article/... --input draft-article.json --execute --require-images
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\shopify-blog-admin.mjs verify --env skill-hub.env --article-id gid://shopify/Article/...
```
...
```text
node C:\Users\qiuru\.agents\skills\wechat-to-shopify-blog\scripts\related-product-block.mjs --product selected-product.json --primary-domain <primary-domain> --heading "Related product"
```
