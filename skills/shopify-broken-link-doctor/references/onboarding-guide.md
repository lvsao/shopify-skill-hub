...
<!-- onboarding-contract: 1.2.0; source-sha256: 4c3c0a9b0bf0e62b51e9600199753fcc2b08897abdb8628e484d437ed1a72eb5 -->
# Connect Your Store

Use this guide only after the merchant reviews a public audit and explicitly requests a selected redirect repair.

## Choose the smallest access path

The public crawl is read-only and needs no connection. It cannot determine Admin redirect coverage or offer a write until the merchant requests connected review.
...
```text
shopify store auth --store <shop>.myshopify.com --scopes read_online_store_navigation,write_online_store_navigation --json
```
...
Connection never replaces the candidate CSV → row-level approval → `fix-preview` → explicit `fix --execute` boundary. Keep 404s without a relevant destination.
