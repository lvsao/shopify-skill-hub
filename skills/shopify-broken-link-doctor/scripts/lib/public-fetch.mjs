...
function isBlockedIp(value) {
  ...
  if (net.isIPv4(ip)) {
    return [
      [0, 0x00ffffff], [0x0a000000, 0x0affffff], [0x64400000, 0x647fffff],
      [0x7f000000, 0x7fffffff], [0xa9fe0000, 0xa9feffff], [0xac100000, 0xac1fffff],
      [0xc0a80000, 0xc0a8ffff], [0xc6120000, 0xc613ffff], [0xe0000000, 0xffffffff],
...
export function validatePublicUrl(raw, { allowedHosts = [] } = {}) {
  ...
  if (parsed.username || parsed.password) throw new Error("Credential-bearing URLs are blocked.");
...
export async function fetchPublic(raw, init = {}, options = {}) {
  ...
  for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
    ...
    if (redirects === MAX_REDIRECTS) throw new Error("Too many redirects while fetching public URL.");
